...
import { fetchProjects, filterProjects, saveProjects } from "~@/core/modules/freelancer";
...
(async () => {
    ...
    if (!scriptInfo.enable) {
        ...
        logger.info("will run script: %s  next %s seconds", scriptUid, scriptInfo.spinSleepTime);
        setTimeout(async () => {
