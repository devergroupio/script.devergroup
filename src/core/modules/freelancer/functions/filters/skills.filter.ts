...
export default async (projects: IFLProject[], settings: IprojectFilter) => {
    ...
    await Promise.all(
        projects.map(async project => {
            const isPassed = await isPassIgnoredSkills(settings.ignoredSkills, project.jobs);
            if (isPassed) {
...
const isPassIgnoredSkills = async (ignoredSkills: IprojectFilter["ignoredSkills"], projectSkills: string[]) => {
