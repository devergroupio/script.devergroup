...
import { bulkupsertNotTasks, bulkupsertNotTasksVariables } from "~@/graphql/generated/bulkupsertNotTasks";
...
export default (description: string, detectPhases: ITraining["detectPhases"]) => {
...
const isTask = (sentence: string, detectPhases: string[]) => {
    const sortedDetectPhases = _.sortBy(detectPhases, phase => phase.length).reverse();
